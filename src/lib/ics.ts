...
function parseRRule(raw: string): RRule {
  ...
  for (const seg of raw.split(";")) {
    ...
    switch (k.toUpperCase()) {
      case "FREQ":
        if (["DAILY", "WEEKLY", "MONTHLY", "YEARLY"].includes(v)) out.freq = v as RRule["freq"];
        break;
      case "INTERVAL":
...
export function buildIcsForMeeting(m: IcsMeetingInput): string {
  ...
  const end = toIcsDate(
    new Date(new Date(m.startsAt).getTime() + (m.durationMinutes ?? 30) * 60_000).toISOString(),
  );
  ...
  const lines = [
    ...
    ...(m.attendees ?? []).map((a) => `ATTENDEE;CN=${escapeText(a)};RSVP=TRUE:mailto:${a}`),
    "END:VEVENT",
    "END:VCALENDAR",
